...
from app.api import health, receipts, tasks, images, batches, exports, cleaning, dashboard, review_batches, backup_api, scan_errors, settings as settings_api, auth as auth_api, admin_receipts, locations, entry_types, industries, categories, ops, messages as messages_api, reports
...
def create_app() -> FastAPI:
    ...
    # Location + entry-type + industry/category reference data (authenticated list + admin write)
    app.include_router(
        locations.router,
        prefix=settings.API_V1_STR,
    )
